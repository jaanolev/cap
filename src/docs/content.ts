...
export const LLMS_FULL_TXT = `# Add a spend cap to your AI route
...
export default async function handler(req, res) {
  ...
  // AI route proceeds with OpenAI, Anthropic, etc.
}
\`\`\`

## Best Practices

1. **Always check the gate before expensive operations**: Call consume() before invoking LLM APIs
2. **Use idempotency keys for retries**: Prevents double-counting on network failures
3. **Handle 402 responses gracefully**: Return clear error messages to users
4. **Monitor remaining balance**: Show users their remaining quota
5. **Fail closed**: If Cap is unreachable, deny the request rather than allowing unlimited access
...
try {
  ...
  // Fail closed: treat Cap errors as insufficient balance
  console.error('Cap API error:', error);
  return res.status(402).json({ 
    error: 'Payment required - service temporarily unavailable' 
  });
}
\`\`\`
...
Cap is designed as a stateless gate:
- PostgreSQL backend for atomic operations
- Fail-closed design: errors deny access
...
- **Cursor Skill**: https://cap-alpha-one.vercel.app/SKILL.md
- **GitHub**: https://github.com/jaanolev/cap
- **Issues**: https://github.com/jaanolev/cap/issues
`;
